'''
assignment1: SVM
question6: implement a trading strategy based on your labels (from linear SVM) for year 2 and compare the performance with the ”buy-and-hold” strategy. Which strategy results in a larger amount at the end of the year
'''
import os
import pandas as pd
...
if portfolios[-1] > portfolios_buy_hold[-1]:
    print('trading strategy based on linear SVM predicted labels results in a larger amount at the end of the year')
elif portfolios[-1] == portfolios_buy_hold[-1]:
    print('trading strategy based on linear SVM predicted labels results the same amount with buy hold strategy at the end of the year')
else:
...
plt.legend(['trading with labels (linear SVM)','buy hold'])
plt.show()
